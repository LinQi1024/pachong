...
def main():
    ...
    try:
        print '正在连接redis'
        rediscli = redis.StrictRedis(host='192.168.2.101', port=6379, db=0)
        print 'redis连接成功'
        # 指定mysql数据库
        print '正在连接mysql'
    ...
    while True:
        source, data = rediscli.blpop(["fiveone:items"])
        ...
        try:
            ...
            sql = '''insert into job values(null,"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")on duplicate key update ''' \
                  ...
                  (item['name'], item['co_name'], item['area'], item['salary'],item['exp'],item['edu'], item['num'], item['time'],
                   item['otherq'],item['welfare'],item['info'], item['local'], item['co_url'], item['co_type'],'job51')
